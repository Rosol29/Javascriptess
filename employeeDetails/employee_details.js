const employees = [
  ...
  function displayEmployees(){
    const totalEmplyees=employees.map(employee=>`<p> ${employee.id}: ${employee.name} - ${employee.department}- $${employee.salary}</p>`).join('');
    document.getElementById('employeesDetails').innerHTML=totalEmplyees;
...
function findEmployeeById(employeeId){
  ...
  function findSpecialization(specializaton){
    const findSpecialization=employees.find(employee=>employee.specialization === specializaton)
    if(findSpecialization){
        ...
        document.getElementById('employeesDetails').innerHTML='no employee has been found with this specializtion'
    }
}